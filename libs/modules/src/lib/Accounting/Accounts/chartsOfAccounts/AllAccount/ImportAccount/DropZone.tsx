...
type DropZoneProps = {
  ...
  onFileSelected: (file: File) => void; // Add onFileSelected prop
  onFileRemoved: () => void;
};

const DropZone: React.FC<DropZoneProps> = ({ setHeaders,setList,onFileSelected, // Add onFileSelected prop
onFileRemoved, }) => {
  ...
  const onDeleteUploadFile = (file: File) => {
    const newUploadedFiles = dropzone.acceptedFiles.filter(
