...
const CostOfGoodsSoldTable: React.FC<TableProps> = ({
  data,
  handleOpen,
  handleEdit,
}) => {
  ...
  return (
    <Grid container sx={{ py: 1 }}>
      ...
      {data.length === 0 ? (
        <COAGrid2 item xs={12}>
          <COAParagraph>{messages['COA.COGSNoEntries'] as string}</COAParagraph>
        </COAGrid2>
      ) : (
        <>
      ...
      <Grid item xs={12}>
        <Button
          onClick={() => handleOpen('costofGoodsSold')}
          variant="text"
          sx={{ color: '#57B8C9' }}
        >
          <COAAddIcon />
...
export default CostOfGoodsSoldTable;
