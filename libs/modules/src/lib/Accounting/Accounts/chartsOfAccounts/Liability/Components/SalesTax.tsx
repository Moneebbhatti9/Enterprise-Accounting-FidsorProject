...
const SalesTax: React.FC<TableProps> = ({ data, handleOpen, handleEdit }) => {
  ...
  return (
    <Grid container sx={{ py: 1 }}>
      ...
      {data.length === 0 ? (
        <COAGrid2 item xs={12}>
          <COAParagraph>{messages['COA.STNoEntries'] as string}</COAParagraph>
        </COAGrid2>
      ) : (
        <>
