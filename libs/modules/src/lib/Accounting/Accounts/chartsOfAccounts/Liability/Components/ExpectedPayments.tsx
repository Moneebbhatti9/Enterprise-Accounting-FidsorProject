...
const ExpectedPayments: React.FC<TableProps> = ({
  data,
  handleOpen,
  handleEdit,
}) => {
  ...
  return (
    <Grid container sx={{ py: 1 }}>
      <COAGrid1 item xs={12}>
        <SubSectionDiv>
          <SubSectionHeading>
            {messages['COA.EPVTitle'] as string}
            <COAHelpIcon />
          </SubSectionHeading>
        </SubSectionDiv>
      </COAGrid1>
      {data.length === 0 ? (
        <COAGrid2 item xs={12}>
          <COAParagraph>{messages['COA.EPVNoEntries'] as string}</COAParagraph>
        </COAGrid2>
      ) : (
        <>
      ...
      <Grid item xs={12}>
        <Button
          onClick={() => handleOpen('expectedPayments')}
          variant="text"
          sx={{ color: '#57B8C9' }}
        >
          <COAAddIcon />
