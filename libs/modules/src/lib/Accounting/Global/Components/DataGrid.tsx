...
const StyledLink = styled(Link)`
  ...
  color: #111827; /* Default link color */

  &:hover {
    color: #0a8fdc; /* Color on hover */
  }
`;
...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const getDateFormatFromLocalStorage = () => {
    ...
    // 'MM/dd/yyyy' is the default format in case nothing is found in local storage
  };
  const [selectedType, setSelectedType] = useState(() => {
    const storedValue = localStorage.getItem('selectedType');
    return storedValue || 'Standard'; // Use 'type2' as default if no value is stored
  });

  // Function to format the date based on the retrieved date format
  const formatDate = (date: string) => {
    const date1 = new Date(date); // Convert the date string to a Date object
    if (selectedType === 'Friendly') {
      // const formattedDate = format(date1, 'MM/dd/yyyy');
      const timeAgo = formatDistanceToNow(date1, { addSuffix: true });
      return timeAgo;
    } else {
      const dateFormat = getDateFormatFromLocalStorage();
      // Use date-fns to format the date based on the dateFormat
  ...
  const actionsColumn: GridColDef<DataItem> = {
    ...
    renderCell: (params: any) => (
      <div>
        {name === 'All Account' ? (
          <AccountsAction customerId={params.row.id} name={name} link={link} />
        ) : (
          <Actions customerId={params.row.id} name={name} link={link} />
        )}
      </div>
    ),
  };
  ...
  const statusColumn: GridColDef = {
    ...
    renderCell: (params: any) => {
      const bgcolor =
        params.value === 'Active' || params.value === 'Paid'
          ? '#0080003d'
          : params.value === 'Sent'
          ? '#2997ff3d'
